...
void  OSInitHookBegin (void)
{
    ...
    size = OS_CPU_EXCEPT_STK_SIZE;//128
    while (size > 0u) {
        size--;
       *pstk++ = (OS_STK)0;
    }
...
void  OS_CPU_SysTickInit (INT32U  cnts)
{
	...
	//SysTick重装载数值寄存器23:0--RELOAD--倒数计数到0时，将被重载的值
                                                 /* Set prio of SysTick handler to min prio.           */
    OS_CPU_CM3_NVIC_PRIO_ST   = OS_CPU_CM3_NVIC_PRIO_MIN;  //SysTick控制及状态寄存器0xE000E010
    //《Cortex-M3权威指南》-PRI_15-SysTick优先级寄存器--设为0xFF
                                                 /* Enable timer.                                      */
    OS_CPU_CM3_NVIC_ST_CTRL  |= OS_CPU_CM3_NVIC_ST_CTRL_CLK_SRC | OS_CPU_CM3_NVIC_ST_CTRL_ENABLE;
	//第16位:COUNTFLAG:1--SysTick已经计数到0
	//第2位:CLKSOURCE:1--内核时钟（选中）
	//第1位:TICKINT:1--SysTick倒数计数到0时产生SysTick异常请求
	//第0位:ENABLE:1--SysTick定时器的使能位（选中）
                                                 /* Enable timer interrupt.                            */
    OS_CPU_CM3_NVIC_ST_CTRL  |= OS_CPU_CM3_NVIC_ST_CTRL_INTEN;
	//第1位:TICKINT:1--SysTick倒数计数到0时产生SysTick异常请求（选中）
	
	//可以添加时钟校准值，SysTick校准数值寄存器0xE000E01C，别名OS_CPU_CM3_NVIC_ST_CAL
	//31:NOREF--(只读)1--没有外部参考时钟
	//30:SKEW--(只读)1--校准值不是准确的10ms
	//23:0:TENMS--10ms的间隔中倒计数的格数(为得到恒定的SysTick中断频率，可以直接把TENMS写入重载寄存器，或者按比例写入。)
}
