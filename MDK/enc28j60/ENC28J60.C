...
/****************************************************************************
* 名    称：void enc28j60PhyRead(unsigned char address)
* 功    能：读出指定PHY寄存器数值
* 入口参数：
* 出口参数：无
* 说    明：
* 调用方法：
****************************************************************************/ 
uint16_t enc28j60PhyRead(unsigned char address)
{
	uint16_t phy_dat;
	
	enc28j60Write(MIREGADR, address);
	enc28j60Write(MICMD, 0x01);

	while(enc28j60Read(MISTAT)&0x01);
	//当MIRD数据被更新，BUSY位由硬件清零
	enc28j60Write(MICMD, 0x00);

	phy_dat = (0xFFFF&(enc28j60Read(MIRDH)<<8)) | enc28j60Read(MIRDL);
...
//元。
	...
	//bit 6 PKTIE： 接收数据包待处理中断允许位
		//1 = 允许接收数据包待处理中断
		//0 = 禁止接收数据包待处理中断
	//使用连接状态中断，必须先开启PHIE的PLINKIE、PGEIE中断
	enc28j60PhyWrite(PHIE, 0x0012);
	enc28j60WriteOp(ENC28J60_BIT_FIELD_SET, EIE, EIE_INTIE|EIE_RXERIE|EIE_LINKIE);	//EIE_INTIE|EIE_PKTIE|EIE_RXERIE|EIE_LINKIE
